...
| `ATE101` | Offline mode could not find a catalog. | Run without `--offline` to download the official catalog, or pass `--catalog` with an existing ATE JSONL or CSV file. |
...
| `ATE104` | The selected catalog could not be read. | Pass `--catalog` with a valid ATE JSONL or CSV file. |
